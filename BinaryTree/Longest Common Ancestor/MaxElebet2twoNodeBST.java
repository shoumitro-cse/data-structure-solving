...
//javac -d classes MaxElebet2twoNodeBST.java  && cd classes && java MaxElebet2twoNodeBST && cd ..

class MaxElebet2twoNodeBST { 
	...
	static int maximumElement( Node root, int x, int y) { 
		...
		return Math.max(maxelpath(p, x), maxelpath(p, y)); //(9, 12)
	} 
...
Create a Binary Search Tree by inserting element from arr[0] to arr[n-1]. The task is 
to find the maximum element in the path from A to B.
