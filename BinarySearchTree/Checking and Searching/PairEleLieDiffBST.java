...
//javac -d classes PairEleLieDiffBST.java  && cd classes && java PairEleLieDiffBST && cd ..

import java.util.*; 

class PairEleLieDiffBST 
{ 
	// A binary Tree node 
	static class Node 
	{ 
	...
	static void pairSumUtil(Vector<Integer> vect1, Vector<Integer> vect2, int sum) { 
		// Initialize two indexes to two different corners of two Vectors. 
		int left = 0; 
		...
		while (left < vect1.size() && right >= 0) { 
			...
			// If sum is more, move to higher value in first Vector. 
			else if (vect1.get(left) + vect2.get(right) < sum) 
				left++; 
			// If sum is less, move to lower value in second Vector. 
			else
				right--; 
		} 
	} 
	...
	{ 
		// Store inorder traversals of two BSTs in two Vectors. 
...
Given two Binary Search Trees (BST) and a given sum. The task is to find pairs 
with given sum such that each pair elements must lie in different BST.
