// C++ program to find all the reachable nodes for every node present in arr[0..n-1].

// Find all reachable nodes from every node present in a given set

// g++ find_all_reachable_nodes.cpp && ./a.out
...
Given an undirected graph and a set of vertices, find all reachable nodes from every 
vertex present in the given set.
