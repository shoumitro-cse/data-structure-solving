...
// javac -d classes  MakeEulerCircuit.java  && cd classes && java MakeEulerCircuit && cd ..

import java.io.*; 
import java.util.*; 

class MakeEulerCircuit { 
	...
	// Return minimum edge required to make Euler Circuit 
	static int minEdge(int n, int m, nt[] s, int[] d) { 
		// g : to store adjacency list representation of graph. 
		...
		for (int i = 1; i <= n; i++) { 
			if (vis[i] == 0) { 
				...
				// Checking if connected component is odd. 
				if (odd[comp] == 0) {
					e.add(comp); 
				} else {
				   // Checking if connected component is even. 
					o.add(comp); 
				}
			} 
		} 
		
		// If whole graph is a single connected component with even degree. 
		...
		// If graph have atleast one even connected component 
		if (e.size() != 0) 
			ans += e.size(); 
...
Given a undirected graph of n nodes and m edges. The task is to find minimum edges required to 
make Euler Circuit in the given graph.
