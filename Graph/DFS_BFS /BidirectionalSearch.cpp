// C++ program for Bidirectional BFS search to check path between two vertices

//Bidirectional Search

// g++ BidirectionalSearch.cpp && ./a.out

#include <bits/stdc++.h>
using namespace std;

// class representing undirected graph using adjacency list
class Graph {
...
void Graph::BFS(list<int> *queue, bool *visited, int *parent) {
	...
	for (i = adj[current].begin(); i != adj[current].end(); i++) {
		// If adjacent vertex is not visited earlier mark it visited by assigning true value
		if (!visited[*i]) {
...
int Graph::isIntersecting(bool *s_visited, bool *t_visited) {
	...
	int intersectNode = -1;
	for(int i = 0; i < V; i++) {
		// if a vertex is visited by both front and back BFS search return that node else return -1
