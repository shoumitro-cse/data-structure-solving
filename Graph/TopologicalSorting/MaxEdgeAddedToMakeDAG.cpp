...
// g++ MaxEdgeAddedToMakeDAG.cpp && ./a.out
...
vector<int> Graph::topologicalSort() {
	...
	while (!q.empty()) {
		...
		for (list<int>:: iterator j = adj[t].begin(); j != adj[t].end(); j++) {
			...
			// if indegree becomes 0, just push into queue
			if (indegree[*j] == 0) {
				q.push(*j);
			}
...
// The function prints all edges that can be added without making any cycle
...
after which new graph still remain a DAG that means the reformed graph should have maximal 
number of edges, adding even single edge will create a cycle in graph.
