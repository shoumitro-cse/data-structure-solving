...
// g++ UnionIntersectionHashing.cpp && ./a.out
#include <bits/stdc++.h> 
using namespace std; 
...
void storeElement(struct Node* head1, struct Node* head2, unordered_map<int, int>& eleOcc) { 
...
/* Function to get the union of two linked lists head1 and head2 */
...
/* Function to get the intersection of two linked lists head1 and head2 */
struct Node* getIntersection( unordered_map<int, int> eleOcc) { 
	struct Node* result = NULL; 
	// Push a node with an element having occurrence of 2 as that 
	// means the current element is present in both the lists 
	for (auto it = eleOcc.begin(); it != eleOcc.end(); it++) {
		if (it->second == 2) {
		   push(&result, it->first); 
		}
...
// Prints union and intersection of lists with head1 and head2. 
void printUnionIntersection(Node* head1, Node* head2) { 
	...
	storeElement(head1, head2, eleOcc); 
