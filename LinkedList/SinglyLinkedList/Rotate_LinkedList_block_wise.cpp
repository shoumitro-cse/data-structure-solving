...
// g++ Rotate_LinkedList_block_wise.cpp && ./a.out
...
Node* rotateHelper(Node* blockHead, Node* blockTail, int d, Node** tail, int k) { 
	...
	if (d > 0) { 
		Node* temp = blockHead; 
		for (int i = 1; temp->next->next && i <= k - 2; i++) {
			temp = temp->next; 
		}
...
Node* rotateByBlocks(Node* head, int k, int d) {
	...
	Node* block_end = head, *tail = NULL; 
	// Traverse upto last element of this block 
	int i; 
	for (i = 1; block_end->next && i <= k-1; i++) {
		block_end = block_end->next; 
	}
	// storing the first node of next block 
	Node* nextBlock = block_end->next; 

	// If nodes of this block are less than k. Rotate this block also 
	if (i < k) 
		head = rotateHelper(head, block_end, d % k, &tail, i); 
	else
		head = rotateHelper(head, block_end, d % k, &tail, k); 
...
Given a Linked List of length n and block length k rotate in circular manner 
towards right/left each block by a number d. If d is positive rotate towards right
 else rotate towards left.
