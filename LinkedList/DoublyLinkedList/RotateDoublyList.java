...
//javac -d classes RotateDoublyList.java  && cd classes && java RotateDoublyList && cd ..

class RotateDoublyList { 
	...
	static void rotate( int N) { 
		...
		// for example N = 2 and list = a <-> b <-> c <-> d <-> e. 
		Node current = head; 
		...
		// it in a variable. NthNode points to node 'b' in the above example 
		Node NthNode = current; 

		// current will point to last node 
		// after this loop current will point to node 'e' in the above example 
