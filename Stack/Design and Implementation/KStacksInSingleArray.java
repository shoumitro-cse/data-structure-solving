...
// javac -d classes KStacksInSingleArray.java  && cd classes && java KStacksInSingleArray && cd ..
...
{ 
	...
	static class KStack {
		...
		// To push an item in stack number 'stack_no' where stack_no is from 0 to k-1 
		void push(int item, int stack_no) {
			...
			next[i] = top[stack_no]; // i=0
			top[stack_no] = i; 
			// Put the item in array 
			arr[i] = item; 
		} 

		// To pop an from stack number 'stack_no' where stack_no is from 0 to k-1 
		int pop(int stack_no) { 
			...
			// Find index of top item in stack number 'stack_no' 
			int i = top[stack_no]; 
		...
		// To check whether stack number 'stack_no' is empty or not 
