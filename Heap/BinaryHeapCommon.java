...
class BinaryHeapCommon { 
	...
	public BinaryHeapCommon(int a[], int size) 
	{ 
		heap_size = size; 
		capacity = size; 
		harr = a; 
	...
	void BinaryHeapCommonify(int i) 
	{ 
		int l = left(i); 
		int r = right(i); 
		int smallest = i; 
	...
	{ 
		...
		BinaryHeapCommon BinaryHeapCommon = new BinaryHeapCommon(len, n); 

		// Iterate while size of heap doesn't become 1 
		while (!BinaryHeapCommon.isSizeOne()) { 
			// Extract two minimum length ropes from min heap 
			int min = BinaryHeapCommon.extractMin(); 
			int sec_min = BinaryHeapCommon.extractMin(); 

			cost += (min + sec_min); // Update total cost 

			// Insert a new rope in min heap with length equal to sum 
			// of two extracted minimum lengths 
			BinaryHeapCommon.insertKey(min + sec_min); 
		} 
