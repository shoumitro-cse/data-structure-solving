...
void restoreUp(int arr[], int index, int k) { 
	...
	// element inserted is the maximum restore up will send it to the root node 
	while (parent>=0) { 
...
void buildHeap(int arr[], int n, int k) { 
	...
	// non-leaf node all the way upto the root node and calling restore down on each 
	for (int i= (n-1)/k; i>=0; i--) 
		restoreDown(arr, n, i, k); 
} 

// Function to insert a value in a heap. Parameters are 
// the array, size of heap, value k and the element to be inserted 
...
// the heap and then restores the heap property of the remaining nodes 
