...
//javac -d classes LargestSubarrContiguous.java  && cd classes && java LargestSubarrContiguous && cd ..

import java.util.*; 

class LargestSubarrContiguous {
...
Given an array of integers, find length of the longest subarray which contains numbers that 
can be arranged in a continuous sequence.

In the previous post, we have discussed a solution that assumes that elements in given array are 
