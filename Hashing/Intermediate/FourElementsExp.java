...
//javac -d classes FourElementsExp.java  && cd classes && java FourElementsExp && cd ..

import java.io.*; 
import java.util.*; 

class FourElementsExp {
		...
		for (int i=0; i<n; ++i) { 
			for (int j=i+1; j<n; ++j) { 
				...
				if (!map.containsKey(sum)) {
				    ...
				    // Else (Sum already present in hash) Find previous pair 
					pair p = map.get(sum); 
	...
	public static void main(String args[]) { 
		...
		FourElementsExp a = new FourElementsExp(); 
		a.findPairs(arr); 
	} 
} 
